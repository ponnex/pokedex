<template>
	<div>
		<div
			v-show="getBgColor() && isImageLoaded"
		...
		>
			...
			<img
				v-else
				ref="pokemon-image"
				class="h-24 p-2 justify-self-center self-center col-span-1"
				:src="pokemonImage()"
				:alt="pokemon.name"
				@load="onImageLoad()"
			>
		...
		<div
			v-if="!getBgColor() && !isImageLoaded"
			class="relative inline-grid grid-cols-3 gap-x-4 rounded-2xl min-w-full h-24 text-white dark:text-black shadow cursor-pointer"
		>
			<div class="h-16 w-16 p-2 justify-self-center self-center col-span-1 rounded bg-gray-200"></div>
			<div class="flex flex-col col-span-2">
				<div class="font-medium h-4 mt-5 text-2xl w-11/12 rounded bg-gray-100"></div>
				<div class="flex space-x-2 mt-3">
					<div class="h-5 w-5 self-center flex p-1 gap-x-1 rounded-full bg-gray-100"></div>
					<div class="h-5 w-5 self-center flex p-1 gap-x-1 rounded-full bg-gray-100"></div>
				</div>
			</div>
		</div>
